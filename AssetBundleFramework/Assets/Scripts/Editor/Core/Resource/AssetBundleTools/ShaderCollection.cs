...
public class ShaderCollection// : EditorWindow
{
    ...
    public static string GenShaderVariant(string collectfolderpath, string outputfolderpath)
    {
        var assetsindex = collectfolderpath.IndexOf("Assets");
        var collectrelativepath = collectfolderpath.Substring(assetsindex, collectfolderpath.Length - assetsindex);
        var outputassetsindex = outputfolderpath.IndexOf("Assets");
        var outputrelativepath = outputfolderpath.Substring(outputassetsindex, outputfolderpath.Length - outputassetsindex);
        Debug.Log(string.Format("Shader变体搜集全路径:{0},Asset目录路径:{1}", collectfolderpath, collectrelativepath));
        if (!Directory.Exists(collectfolderpath))
        {
            Debug.LogError(string.Format("Shader变体搜集路径目录:{0}不存在!", collectfolderpath));
            return null;
        }

        if(!Directory.Exists(outputfolderpath))
        {
            Debug.Log(string.Format("Shader变体文件输出目录不存在:{0},新创建一个!", outputfolderpath));
        ...
        var _p = outputrelativepath + "/DIYShaderVariantsCollection.shadervariants";
        Debug.Log(string.Format("Shader变体搜集文件输出目录:{0}", _p));
