...
public class ResourceDebugWindow : BaseEditorWindow
{
    ...
    private void displayOneAssetBundleLoaderInfoUI(AssetBundleLoader abl)
    {
        ...
        for (int i = 0, length = abl.DepAssetBundleInfoList.Count; i < length; i++)
        {
            ...
            EditorGUILayout.LabelField(i + ". " + depabi.ResourcePath, GUILayout.Width(150.0f));
        }
        EditorGUILayout.EndHorizontal();
    ...
    {
        EditorGUILayout.BeginHorizontal();
        EditorGUILayout.LabelField(string.Format("资源名 : {0}", abi.ResourcePath), GUILayout.Width(500.0f));
